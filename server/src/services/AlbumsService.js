...
class AlbumsService{
    async editAlbum(albumId, editedData, userId) {
        ...
        if(album.creatorId != userId){ throw new Forbidden("you cant edit another users album!")}
    ...
    async deleteAlbum(albumId, userId) {
        const album = await this.getAlbumById(albumId)
        if(album.creatorId != userId){ throw new Forbidden("you cant delete another users album!")}
        album.isArchived = true
        await album.save()
        return `Album ${album.name} has been archived!`
    }
    async getAlbumById(albumId) {
        const album = await dbContext.Album.findById(albumId).populate('creator', 'name picture')
        if(album==null) throw new BadRequest('this album does not exist')
        return album
    }
    async getAllAlbums() {
        const albums = await dbContext.Album.find().populate('creator', 'name picture')
        return albums
    }
