...
export class ProfilesController extends BaseController {
  ...
  async getTagsByProfileId(request, response, next) {
    try {
      const profileId = request.params.id;
      const albumTagsCount =
        await albumTagsService.getAlbumTagsCountByProfileId(profileId); //these are intense
      const photoTagsCount =
        await photoTagsService.getPhotoTagsCountByProfileId(profileId);
